...
where
    F: 'static + FnMut(&[u8], &EncodedVideoFrame, &EncodedVideoFrames) + Send,
{
                ...
                match data {
                    MediaData::VideoFrame(vf) => {
                        if let Ok(true) = video_handler.handle_frame(vf) {
                            video_callback(&video_handler.rgb, &video_handler.frame, &video_handler.frames);
                        }
                    }
                    MediaData::Reset => {
                        video_handler.reset();
